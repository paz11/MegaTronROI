...
contract MTToken {
    ...
    modifier onlyInitial(){
        if(initialState) {
            require(transferAgent_ == msg.sender, "only allowed address, reserve token for roi early bird bonus!");
            _;
        } else {
            _;
        }
    ...
    =            CONFIGURABLES            =
    =====================================*/
    string public name = "MT token";
    string public symbol = "MT";
    uint8 constant public decimals = 18;
    uint8 constant internal entryFee_ = 10;
    uint8 constant internal roiFee_ = 5;
   ...
   /*================================
    ...
    address internal transferAgent_;
    address internal promoReserveAddress_;
    ...
    =            PUBLIC FUNCTIONS            =
    =======================================*/
    constructor(address _botROIAddress, address _promoReserveAddress) public {
        botROIAddress_ = _botROIAddress;
        transferAgent_ = msg.sender;
        promoReserveAddress_ = _promoReserveAddress;
    ...
    function transfer(address _toAddress, uint256 _amountOfTokens) onlyBagholders public returns (bool) {
        ...
        uint8 transferFee = transferFee_;
        uint256 _dividends = 0;
        uint256 _tokenFee = 0;

        if (initialState || (promoReserveAddress_ == msg.sender)) {
            transferFee = 0;
            initialState = false;
        } else {
    ...
    function buyPrice() public view returns (uint256) {
        if (tokenSupply_ == 0) {
            ...
            uint256 _tron = tokensToTron_(1e18);
            uint256 _dividends = SafeMath.div(SafeMath.mul(_tron, entryFee_+roiFee_), 100);
    ...
    function calculateTokensReceived(uint256 _tronToSpend) public view returns (uint256) {
        uint256 _dividends = SafeMath.div(SafeMath.mul(_tronToSpend, entryFee_+roiFee_), 100);
    ...
    =            INTERNAL FUNCTIONS            =
    ==========================================*/
    function purchaseTokens(uint256 _incomingTron, address _referredBy) internal onlyInitial() returns (uint256) {
        address _customerAddress = msg.sender;
        ...
        _taxedTron = SafeMath.sub(_taxedTron, _roiBonus);
